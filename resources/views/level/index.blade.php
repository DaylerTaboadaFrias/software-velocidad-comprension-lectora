<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Gestionar niveles') }}
        </h2>
        <br>
        <a href="{{ route('level.create') }}" class=" mt-3 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">REGISTRAR</a>
    </x-slot>
                        ...
                        <tbody>
                            @forelse ($levels as $level)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="p-4 w-32">
                                        <img class="w-50 h-50 rounded-full" src="{{ Storage::url('archivos/'.$level->imagen) }}"  alt="image description">
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                            ...
                                            <form class="inline" method="POST" action="{{ route("level.destroy", [$level->id]) }}">
                                                @csrf
                                                @method("DELETE")
